...
extern int     InpMaxOpenTrades      = 15;  // Raised from 5 for grid capacity           // Max concurrent trades allowed across all models (changed from 3)
...
extern bool   InpEnableCooldownSystem  = false;
extern bool   InpEnable_ReaperConditionFilter = false;  // NEW: default OFF // <<< ADD THIS NEW INPUT AND SET TO false
...
PerfData g_perfData[10]; // 0=MR, 2=Titan, 3=Warden, 4=Reaper, 5=Silicon-X, 6=Micro, 7=Chronos, 8-9=RESERVED
...
int OnInit()
{
   ...
   g_perfData[7].name = "Chronos";
   g_perfData[8].name = "Reserved";
   g_perfData[9].name = "Reserved";
   // ---
...
void ExecuteMeanReversionModelV8_6()
{
   ...
   {
      LogError(ERROR_INFO, "ExecuteMeanReversionModelV8_6: SKIPPED - Reaper market conditions not met", "ExecuteMeanReversionModelV8_6");
      return;
   }
      ...
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
      {
         if(OrderSymbol() == Symbol())
         {
            ...
            if(magic == InpMagic_MeanReversion ||
               ...
               magic == InpChronos_MagicNumber)  // includes Chronos
            {
               count++;
            }
      }
...
void OnTick_Elite() {
    ...
    // MOVED TO OnNewBar(): if(InpSiliconX_Enabled) ExecuteSiliconCore();
