//+------------------------------------------------------------------+
//|        DESTROYER_QUANTUM_V26.0_MATH_FIRST_SIGNAL_ENGINE.mq4     |
//|                    Copyright 2026, Quantum Leap Analytics        |
//|  DESTROYER QUANTUM V26 - MATH-FIRST SIGNAL GENERATION (BREAKING THE V18 BINARY CAP)         |
//|                     https://github.com/okyyryan                  |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Quantum Leap Analytics"
#property link      "https://github.com/okyyryan"
#property version   "26.00"  // V26.0 MATH-FIRST ENGINE: Pure Math Signal Generation + Full V25 Fixes
#property strict
...
extern double  InpBase_Risk_Percent    = 0.5;         // Base risk per trade as a percentage
...
extern int    InpDualMomentum_Cooldown     = 3;            // Bar cooldown between trades
extern bool   InpDualMomentum_Allow_Defensive = true;     // Allow trading in defensive hive state
...
extern int    InpReaper_MaxLevels       = 10;         // Maximum grid levels per basket (safety limit)
...
extern double InpSX_LotExponent             = 1.6;          // Geometric lot progression multiplier.
//--- Grid Mechanics
extern int    InpSX_MaxLevels               = 18;           // Capped 18-level grid to prevent runaway exposure.
    ...
    if(g_strategy_win_rates[strategy_index] == 0)
    {
        ...
        switch(strategy_index)
        {
            case 0: // Mean Reversion - historically good performance
                ...
                break;
            case 1: // Dual-Momentum Breakout (V26.1) - Donchian20+MA200, validated
                g_strategy_win_rates[strategy_index] = 0.69;   // WR 69% from EURUSD H4 backtest
                g_strategy_avg_wins[strategy_index] = 1.57;    // Avg Win / Avg Loss ratio ≈ 1.57:1
                g_strategy_avg_losses[strategy_index] = 1.0;
                break;
...
void OnNewBar()
{
   ...
   // STRATEGY: Dual-Momentum Breakout (Trend/Breakout Hybrid)
   if(InpDualMomentum_Enabled)
   {
      ExecuteDualMomentumBreakout();
      if(CountOpenTrades() >= InpMaxOpenTrades) { if(!IsOptimization()) UpdateDashboard_StaticV8_6(); return; }
   }
...
if(!InpDualMomentum_Enabled) return;
   ...
   // State-based permission check
   if (g_hive_state == HIVE_STATE_DEFENSIVE && !InpDualMomentum_Allow_Defensive)
   {
      LogError(ERROR_INFO, "ExecuteDualMomentumBreakout: SKIPPED - Strategy not allowed in defensive mode", "ExecuteDualMomentumBreakout");
      return;
   }
   ...
   // INDICATORS
   // ========================================
   int donchianLen = InpDualMomentum_DonchianLen;
   int maLen       = InpDualMomentum_MA_Period;
   int atrLen      = InpDualMomentum_ATR_Period;

   if(Bars < maLen + 10) return; // Wait for enough data

   double donchianHigh = High[iHighest(Symbol(), Period(), MODE_HIGH, donchianLen, 1)];
   double donchianLow  = Low [iLowest(Symbol(), Period(), MODE_LOW,  donchianLen, 1)];
   double maPrice      = iMA(Symbol(), Period(), maLen, 0, MODE_EMA, PRICE_CLOSE, 1);
   double atrVal       = iATR(Symbol(), Period(), atrLen, 1);
   ...
   int dir = longSignal ? OP_BUY : OP_SELL;
   double entryPrice = longSignal ? Ask : Bid;

   // Stop loss & take profit in POINTS (point-based prices)
   double slPoints = InpDualMomentum_SL_Mult * atrVal / Point;   // e.g., 1.5 * ATR => points
   double tpPoints = InpDualMomentum_TP_Mult * atrVal / Point;
   
   double sl = longSignal ? entryPrice - slPoints * Point : entryPrice + slPoints * Point;
   double tp = longSignal ? entryPrice + tpPoints * Point : entryPrice - tpPoints * Point;

   // Convert stop loss to PIPS for lot sizing & V23 tracking (1 pip = 10 points on 5-digit)
   double stopLossPips = slPoints / 10.0;
   double lots = GetLotSize_Ascension(stopLossPips, 1); // Strategy index 1 for DualMomentum

   if(lots <= 0) {
      LogError(ERROR_INFO, "DualMomentum: Lot size zero — risk blocked by Ascension", "ExecuteDualMomentumBreakout");
      return;
   }
   ...
   int ticket = RobustOrderSend(
       ...
       sl,
       tp,
       "DualMomentum_Breakout",
       InpDualMomentum_MagicNumber
   );

   if(ticket > 0)
   {
      lastDMTrade = TimeCurrent();
      V23_OnTradeOpen(ticket, stopLossPips, 0, 0); // Track as new worker
      LogError(ERROR_INFO, "DualMomentum: Trade opened " + (dir==OP_BUY?"BUY":"SELL") +
                    " Lots=" + DoubleToString(lots,2) +
                    " Entry=" + DoubleToString(entryPrice,5) +
                    " SL=" + DoubleToString(sl,5) +
                    " TP=" + DoubleToString(tp,5), "ExecuteDualMomentumBreakout");
   }
   else
   {
      LogError(ERROR_WARNING, "DualMomentum: OrderSend failed, err=" + IntegerToString(GetLastError()), "ExecuteDualMomentumBreakout");
   }
}
...
void ExecuteTitanStrategy()
{
    ...
    if(currentATR < volatilityThreshold || currentVolatilityPercentile < 0.4) // PHOENIX: Lowered from 0.6
    {
    ...
    if (atr1 < (atr2+atr3+atr4)/3.0)
    {
        LogError(ERROR_INFO, "Titan Blocked (Valkyrie): Volatility is not expanding.");
        return; // Block trade if volatility is contracting or stable.
    }
...
{
    if(Period() != PERIOD_H4) return;
    // if(!InpWarden_Enabled) return; // LEVIATHAN: All strategies enabled
   ...
   if(InpEnable_ReaperConditionFilter && !IsReaperConditionMet())
    ...
    if(isSqueezeOn)
    {
        ...
        // V10.0: Enhanced Breakout Confirmation Filter
        double breakout_bar_range = High[1] - Low[1];
        double avg_bar_range = iATR(Symbol(), Period(), 10, 1); // Using ATR(10) as a proxy for average range
        bool breakout_confirmed = (breakout_bar_range > avg_bar_range);

        // V10.0: Volume Confirmation Added
        // Buy Breakout CONFIRMED: Breakout, Momentum, Bar Range, AND Volume Confirmation
        // PHOENIX UPGRADE: Ensure the close is at least 20% of an ATR value beyond the band
        if(Close[1] > bb_upper_break && (Close[1] - bb_upper_break) > (iATR(Symbol(), Period(), 14, 1) * 0.2) && Close[1] > momentum_ma && breakout_confirmed && Volume[1] > Volume[2])
        {
            // PHASE 3: Use Warden's Hard Deck Stop Loss
            ...
            // V17.5: QUANTUM PROBABILISTIC MODEL - Replace with Quantum State Lot Sizing
            double lots = MoneyManagement_Quantum(InpWarden_MagicNumber, InpBase_Risk_Percent);
            
            // PHASE 3: Global Risk Check before trade
            if(Global_Risk_Check(lots, slPoints))
            {
               LogError(ERROR_INFO, "ExecuteWardenStrategy: Quantum Lot Sizing (BUY) = " + DoubleToString(lots, 2) + " (Genetic + VSA optimized)", "ExecuteWardenStrategy");
               if(lots > 0)
               {
                  OpenTrade(OP_BUY, lots, Ask, sl, tp, "WARDEN_BUY_V10.0", InpWarden_MagicNumber);
               }
            }
        }
        // Sell Breakout CONFIRMED: Breakout, Momentum, Bar Range, AND Volume Confirmation
        // PHOENIX UPGRADE: Ensure the close is at least 20% of an ATR value beyond the band
        else if(Close[1] < bb_lower_break && (bb_lower_break - Close[1]) > (iATR(Symbol(), Period(), 14, 1) * 0.2) && Close[1] < momentum_ma && breakout_confirmed && Volume[1] > Volume[2])
        {
            // PHASE 3: Use Warden's Hard Deck Stop Loss
            double slPoints = CalculateStopLoss_Warden();
            ...
            if(Global_Risk_Check(lots, slPoints))
            {
               LogError(ERROR_INFO, "ExecuteWardenStrategy: Quantum Lot Sizing (SELL) = " + DoubleToString(lots, 2) + " (Genetic + VSA optimized)", "ExecuteWardenStrategy");
               if(lots > 0)
               {
                  OpenTrade(OP_SELL, lots, Bid, sl, tp, "WARDEN_SELL_V10.0", InpWarden_MagicNumber);
               }
            }
        }
    }
...
public:
    ...
    bool ApproveTrade(int strategyIndex, double riskAmount, double conviction) {
        ...
        // LAYER 2: PORTFOLIO VAR CHECK
        if(riskAmount > m_portfolioVAR * 0.1) { // Max 10% of portfolio VAR per trade
...
public:
    ...
    void BoostMeanReversionPerformance() {
        ...
        InpMR_RSI_OB = 68.0;  // PHOENIX: Extreme overbought
        InpMR_RSI_OS = 32.0;  // PHOENIX: Extreme oversold
...
public:
    ...
    double GetStrategySpecificRisk(double strategyIndex) {
...
double MoneyManagement_Quantum(int magicNumber, double baseRiskPercent)
{
   ...
   if (magicNumber == 888001 || magicNumber == 888002) riskMultiplier = 3.0; 
   ...
   else if (magicNumber == InpTitan_MagicNumber || magicNumber == 777001 || magicNumber == 555001) riskMultiplier = 0.2;
